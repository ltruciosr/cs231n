...
def adam(w, dw, config=None):
    ...
    learning_rate = config["learning_rate"]*np.sqrt(1-config["beta2"]**config["t"])/(1-config["beta1"]**config["t"])
    next_w = w - learning_rate * config["m"]/(np.sqrt(config["v"]) + config["epsilon"])
